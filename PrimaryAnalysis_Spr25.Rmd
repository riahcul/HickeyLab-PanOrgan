...
blocks <- rbind(
  ...
  data.frame(
    ...
    ymin = c(4, 3),
    ymax = c(5, 4),
    label = c("Unlabeled (12)", "No metadata (33)"),
...
df_wide <- df_wide |>
  mutate(
    total_count = rowSums(across(all_of(cell_type_cols))),
    radius = sqrt(total_count) / max(sqrt(total_count)) * 3  # scale to fit
  )
...
ggplot(data = df_bubble) +
  geom_scatterpie(
    aes(x = year, y = study_count, group = dataset_name, r = 0.00035*count), cols = cell_type, color = NA,
    long_format = TRUE,
  ) + coord_equal() +
  theme_minimal() #+
